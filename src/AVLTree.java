...
public class AVLTree {
	...
	public class AVLNode implements IAVLNode {
		...
		private void rootToMaxCheckUp() {
			AVLNode tmp = this;
			while (tmp.right.rank != -1) { //// go down right to go up and check balance
				tmp = tmp.right;
			}
			tmp.heightCheckInsert();
		...
		private void rootToMinCheckUp() {
			AVLNode tmp = this;
			while (tmp.left.rank != -1) { //// go down left to go up and check balance
				tmp = tmp.left;
			}
			tmp.heightCheckInsert();
			while (tmp.parent != null) {
